...
import { placeTackleAction } from "@/app/(app)/matches/actions";
  ...
  return (
    <div className="relative">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger
          className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-lg border border-lime-400/40 bg-transparent px-3 text-sm font-medium text-lime-300 transition-colors hover:bg-lime-400/10 disabled:pointer-events-none disabled:opacity-50"
          disabled={rivals.length === 0 || loading}
        >
        ...
        <PopoverContent align="center" className="w-80">
          ...
          {rivals.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Aucun autre parieur sur ce match pour l&apos;instant.
            </p>
          ) : (
              ...
              {rivals.map((rival) => {
                ...
                return (
                  <li key={rival.user_id}>
                    <button
                      type="button"
                      disabled={loading || rival.user_id === currentUserId}
                      onClick={() => handleSelect(rival.user_id)}
                      className={cn(
